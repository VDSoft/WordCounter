// <copyright company="VDSoft" file="WordCounter.cs">
...
public static class WordCounter
{
    ...
    /// <returns>
    /// A <see cref="Dictionary{TKey, TValue}" /> containing the words as keys and the count as a value.
    /// </returns>
    /// <exception cref="ArgumentException">Please provide a valid cancellation token - cancellationToken</exception>
    public static Dictionary<string, int> Count(string text, CancellationToken cancellationToken, char separator = ' ')
    {
        ...
        return CountWordsParallel(splittedText, cancellationToken)
            .ToDictionary(k => k.Key, v => v.Value);
    }

    private static string[] SplitTextBySeparator(string text, char separator) => 
